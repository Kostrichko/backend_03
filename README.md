...
Бот также имеет свой config файл `bot/config.py` с аналогичными лимитами:
...
В проекте настроены автоматические проверки (CI/CD):
- **Django Tests**: 54 теста для проверки бизнес-логики и API.
- **Black**: Проверка форматирования кода.
- **isort**: Сортировка импортов.
- **Flake8**: Линтинг кода.
- **mypy**: Статическая проверка типов.

Запуск всех проверок локально из папки `backend/`:
```bash
./run_checks.sh
```
